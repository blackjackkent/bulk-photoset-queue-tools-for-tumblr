...
var MassPostEditorFeatures4 = function () {
	...
	var doubleCheckDisabledMonths = function (
		currentYr,
		lTs,
		fTs,
		jump,
...
*/
		...
		body.appendChild(
			makeBrowseMonthsWidget(currentYr, lTs, fTs, jump, jumpTo)
		);
		...
		(function (alias) {
			window.columns_need_rebuilding = function () {
				...
				if (document.getElementById('p_loaded') !== null) {
					...
					document.getElementById('p_loaded').innerHTML =
						'x' + p.length;
				}
	...
	var rewrite2 = function () {
		...
		(function (alias) {
			window.remove_tags_from_selected_posts = function () {
				...
				var tags2remove = document.querySelectorAll(
					'#tags input:checked'
				);
	...
	var postCountMake = function () {
		...
		if (
			...
			hl.length !== 0 &&
			privateButton.getElementsByTagName('span')[0].innerText !==
				'(Public)'
		) {
			privateButton.getElementsByTagName('span')[0].innerText =
				'(Public)';
		...
		if (
			...
			followButton !== null &&
			typeof followButton !== 'undefined' &&
			followButton.getElementsByTagName('span')[0].innerText !==
				'UnFollow'
		) {
						...
						if (
							...
							(allToSelect.nottype.length > 1 && // capture some
								allToSelect.nottype.indexOf(idToOrigin[id]) !==
									-1 &&
								allToSelect.nottype.indexOf(idToState[id]) ===
									-1)
						) {
							continue; // has type1, but not type2
						} else {
	...
	var populateSelectByWidget = function () {
		...
		if (data.classList.contains('show-only')) {
			selecto.firstChild.innerHTML = alreadyHidden
				? 'ShowAll'
				: 'Hide=!X';
	...
	var makeSnap = function (cat) {
		...
		getResponseText(
			...
			function (re2) {
				...
				var reg2 =
					/Tumblr._init.secure_form_key\s?=\s?['"]([^'"]+)['"]/;
		...
		getResponseText(
			...
			function (re) {
				if (re === 400) {
					...
					title1.appendChild(
						document.createTextNode('Missing Blogs')
					);
					...
					for (i = 0; i < bodyOld.missing.length; i++) {
						a = document.createElement('a');
						a.href =
							'https://' + bodyOld.missing[i] + '.tumblr.com/';
						a.appendChild(
							document.createTextNode(bodyOld.missing[i])
						);
						a.target = '_blank';
						a.setAttribute(
							'title',
...
*/
			...
			snapshotInfo_frag.appendChild(
				document.createTextNode('Snapshot Info')
			);
				...
				if (
					...
					var pauseBeforeNext = function () {
						...
						if (
							...
							var fKey =
								fBrick[0].getAttribute('data-follower-key');
							...
							!data.classList.contains('next_page_false')
						) {
			...
			function (re) {
				...
				var loopFirstBrick = function (an0) {
					...
					if (fanBrick === null) {
						...
						getResponseText(
							...
							function (re2) {
								...
								if (
									...
									if (
										!lcontent.classList.contains('albino')
									) {
										lcontent.classList.add('albino');
									}
									if (lcontent.children.length === 0) {
									...
									var isLike =
										an0.classList.contains('is_like');
									var isReblog =
									...
									var middleChildIB =
										document.createElement('div');
									middleChild.classList.add(
										'overflow-hidden'
									);
									middleChildT.classList.add('links-layer');
									middleChildTR.classList.add('trow');
									middleChildTD.classList.add('tags-layer');
									middleChildIB.classList.add(
										'tag-container'
									);
									middleChildIB.innerHTML =
										'<h2>' + title + '</h2>' + descript;
									...
									link4.addEventListener(
										...
										function (e) {
											...
											var popoverTR =
												document.createElement('div');
											popoverTR.classList.add(
												'fan-pop-tr'
											);
											var popoverTD =
												document.createElement('div');
											popoverTD.classList.add(
												'fan-pop-td'
											);
											var popoverAv =
												document.createElement('div');
											popoverAv.classList.add(
												'fan-pop-av'
											);
											var popoverInner =
												document.createElement('div');
						...
						var nc =
							fanBrick.getElementsByClassName('note_count')[0]; // reblog
						var tc =
							fanBrick.getElementsByClassName('tag_count')[0]; // like count
						if (isReblog) {
							nc.innerText =
				...
				loopFirstBrick(an[0]);
				notesList.last =
					an[an.length - 1].getAttribute('data-timestamp');
	...
	var loadPhotoIntoDOM = function (reloadedImg) {
		...
		clone.setAttribute(
			'title',
			'Copy this caption, to all photo captions.'
		);
		...
		img.addEventListener('load', function () {
			...
			column +=
				(this.height > minBrickHeight ? this.height : minBrickHeight) +
				6;
			data.setAttribute('data-photos_height', column);
			lcontent.style.height = column + 'px';
		});
	...
	var makeBrickFromApiPost = function (post, type, content, index) {
		...
		tagsLayer.addEventListener('click', function (e) {
			...
			this.parentNode.parentNode.parentNode.dispatchEvent(
				clickParentBrick
			);
		});
			...
			getResponseText(
				...
				function (re) {
					...
					if (
						...
						img[uploaded.length].firstChild.src =
							api.response[0].url;
					} // whether we get the URL or not, the show must go on
				...
				function (re1) {
					...
					if (isReblog) {
						...
						if (action !== 'caption' && action !== 'backdate') {
							...
							var mo1 =
								document.getElementById('moleft').value - 1;
							var mo2 =
								document.getElementById('moright').value - 1;
							...
							var pm2 =
								document.getElementById('pmright').checked;
	...
	var openReblogsWidget = function (e) {
		...
		if (nav.classList.contains('open')) {
			...
			wGinput.disabled =
				this.getAttribute('data-default-disabled') === '1';
			wGinput.setAttribute(
				'data-url',
